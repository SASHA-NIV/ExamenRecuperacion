...
public class CambiarClave extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!BCrypt.checkpw(actual, medico.getPassMedi())) {
            response.getWriter().write("La clave actual no es correcta.");
            return;
        }

        try {
            MedicoJpaController ctrl = new MedicoJpaController();
            medico.setPassMedi(BCrypt.hashpw(nueva, BCrypt.gensalt()));
            ctrl.edit(medico);
            response.getWriter().write("ok");
        } catch (Exception e) {
            e.printStackTrace();
            response.getWriter().write("Error al cambiar clave.");
        }
    }
